...
impl<'a> SystemStatus<'a> {
    pub fn new(flags: &'a CPUFlags) -> SystemStatus<'a> {
        SystemStatus {
            cpu_flags: flags,
        }
    }
    ...
    pub fn eval_syscall<T: Fn(u32, u32)>(
        ...
        // When this flag is on, all syscalls return successfully 0
        if self.cpu_flags.syscalls_bypass {
            warn!(
                "Syscall {:?} has been bypassed. No unsafe code was used. Return value is 0.",
                translated_syscall_number
            );
            //registers.write_register(V0, 0); // return value
            registers.write_register(A3, 0); // not an error
            return exit;
        }
        ...
        if translated_syscall_number == SyscallO32::NRUnknown {
            ...
            let result: Result<u32, Error> = match translated_syscall_number {
                ...
                SyscallO32::NRIoctl => {
                    ...
                    let fd = arg1;
                    if self.cpu_flags.block_ioctl_on_stdio && fd < 3 {
                        warn!("IOCTL ignored - manipulating with FD<=2. Returning success.");
                        Ok(0)
                    } else if self.cpu_flags.ioctl_fail_always {
                ...
                SyscallO32::NRGetcwd => {
                    ...
                    if self.cpu_flags.fake_root_directory && buf_size >= 6 {
